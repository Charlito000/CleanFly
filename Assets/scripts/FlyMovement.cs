...
public class FlyMovement : MonoBehaviour
{
    ...
    [Header("Temblor de vuelo")]
    public float amplitudTemblor = 0.08f;
    public float velocidadTemblor = 4f;
    ...
    void ManejarMovimiento()
    {
        ...
        // Nuevo: controla el zumbido según si hay movimiento
        bool seEstaMoviendo = direccion.sqrMagnitude > 0.01f;

        if (AudioManager.instancia != null)
        {
            if (seEstaMoviendo)
                AudioManager.instancia.IniciarVolar();
            else
                AudioManager.instancia.DetenerVolar();
        }
    }
}